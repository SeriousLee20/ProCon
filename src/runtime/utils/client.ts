...
export const createSupabaseClient = () => {
  ...
  const auth = {
    detectSessionInUrl: process.server
      ? false
      : clientOptions.auth?.detectSessionInUrl,
    persistSession: process.server ? false : clientOptions.auth?.persistSession,
    autoRefreshToken: process.server
      ? false
      : clientOptions.auth?.autoRefreshToken,
  };

  // Set auth header
  const options = Authorization
    ? defu({ auth, global: { headers: { Authorization } } }, clientOptions)
    : defu({ auth }, clientOptions);
