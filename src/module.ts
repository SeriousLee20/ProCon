...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    nuxt.hook("nitro:config", (nitroConfig) => {
      ...
      nitroConfig.alias["#supabase/server"] =
        resolveRuntimeModule("./server/services");
    });
    ...
    addTemplate({
      ...
      getContents: () =>
        [
          ...
          `  const serverSupabaseClient: typeof import('${resolve(
            "./runtime/server/services"
          )}').serverSupabaseClient`,
          `  const serverSupabaseServiceRole: typeof import('${resolve(
            "./runtime/server/services"
          )}').serverSupabaseServiceRole`,
          `  const serverSupabaseUser: typeof import('${resolve(
            "./runtime/server/services"
          )}').serverSupabaseUser`,
          "}",
        ].join("\n"),
