  ...
  <div class="bg-bluegray-200 grid px-2 pt-1">
    ...
    <div class="col flex justify-content-end align-content-center gap-1">
      <div v-if="isShowButton" class="flex justify-content-end align-content-center gap-1">
        ...
        <Pdialog v-model:visible="chatDialog" :style="{ width: '30rem', shadow: 'none' }" position="bottomright"
          :draggable="false" class="border-round border-1 border-primary shadow-none bg-white h-max"
          :pt="{ transition: { class: 'transition-none' } }">
          <template #container>
            <div style="height: 25rem; width: 15rem" class="w-full bg-primary-100 h-full">
              ...
              <div class="w-full">
                <div v-if="selectedChatroom.chatlog && !collapseChatroom" class="w-full px-1 overflow-scroll">
                  <div v-for="message in selectedChatroom.chatlog" class="w-full">
                    <div v-if="message.sender_id == user.id" class="flex flex-column justify-content-end pl-8">
                      <div
                        class="font-bold text-sm text-overflow-ellipsis white-space-wrap overflow-hidden text-right pb-1">
                        {{ message.sender_name }}
                      </div>
                      <div
                        class="font-light text-base text-overflow-ellipsis white-space-wrap overflow-hidden border-1 border-white bg-white p-2"
                    ...
                    <div v-else class="w-full h-full">
                      <div v-if="message.text_content" class="pr-8 py-3">
                        ...
                        <div
                          class="font-light text- text-overflow-ellipsis white-space-wrap overflow-hidden border-1 border-white bg-white p-2"
                ...
                <div class="flex align-items-end w-full p-2">
                  <!-- <Pfileupload /> -->
                  <EmojiPicker style="border-radius: 6px;" class="chat w-full" :native="true" @select="onSelectEmoji"
                    picker-type="input" />
                  <span class="pi pi-send flex align-self-center text-lg pl-2"></span>
                </div>
              </div>
            </div>
          </template>
